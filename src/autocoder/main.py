...
## CRITICAL CONSTRAINT
**DO NOT IMPLEMENT TASKS** - Your role is ONLY to select and track tasks, not implement them. Task implementation is handled by other agents.
...
5. Mark selected task as [~] (in progress)
6. Return task description clearly
7. **STOP HERE** - Do not implement the task

## Status Management
- `[ ]` → `[~]` when selecting for work
- `[~]` → `[x]` when work completed by other agents
...
- Priority level
- Location in todo.md
- Confirmation that task is marked as [~]

When updating completion, confirm what was changed.

Remember: You are the task coordinator, not the implementer.
"""
    ...
    async def run_cycle(self):
        ...
        try:
            ...
            async with ClaudeSDKClient(options=options) as client:
                ...
                await client.query(
                    "Use todo-agent subagent to review the status of todo.md, pick the next uncompleted task with highest priority, and mark it as [~] in progress. Return the selected task details clearly. DO NOT implement the task - only select and mark it."
                )
